...
function cuery(_root, _query)
{
	query(_root, _query).then(console.log).catch(console.log);
}

function juery(_root, _query)
{
...
async function query(_root, _query)
{
	async function query(_root, _query, _this)
	{
		...
		if (typeof _query === 'object')
		{
			...
			{
				if (_query.length === 0)
					return await queryFunction(_root, Utils.emptyArray, true, _this);
				...
				if (_query.length === 1)
					if (Array.isArray(_query[0]))
						...
						return await queryFunction(_root, Utils.emptyArray, _query[0], _this);
				...
				var queryResult = {};
				var root = _root instanceof Object ? _root : Utils.emptyObject;
